...
$total_comment_count = $row['comment_count'];
...
$sql = "SELECT * FROM review_comment LEFT JOIN user_tb ON review_comment.writing_user_id = user_tb.id WHERE review_id = $review_id AND comment_class = 0 AND deleteCheck = 0 AND account_delete = 0 order by groupNum,comment_order";
        ...
        while($row = $result->fetch_assoc()){
          ...
          $query = "SELECT COUNT(*) FROM review_comment LEFT JOIN user_tb ON review_comment.writing_user_id = user_tb.id WHERE review_id = $review_id AND comment_class = 1 AND deleteCheck = 0 AND account_delete = 0 AND groupNum = $groupNum";
          $data = mysqli_query($con, $query);
          ...
          $comment_count = $count_row[0];
            ...
            $array = array(
              ...
              "comment_class_child_count" => $comment_count
        ...
        $response['total_comment_count'] = $total_comment_count;
        $response['commentlist'] = $commentList;

        echo json_encode($response);


        //리뷰작성자와 댓글작성자가 다를 때 작동.
        ...
        $ch = curl_init("https://fcm.googleapis.com/fcm/send");
          ...
          $data = json_encode(array(
              ...
              "priority" => "high",
              "data" => array(
                  "title"   => $which_text_choose." 글에 ".$comment_writer_nicname."님이 댓글을 남기셨습니다",
