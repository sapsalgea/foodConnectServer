...
$sql = "SELECT *,(@rownum:=@rownum+1) AS number, (SELECT COUNT( * ) +1 FROM ranking_season_tb WHERE season_name = $now_season AND season_point > t.season_point) AS rank  FROM ranking_season_tb  AS t WHERE season_name = $now_season  ORDER BY season_point";
