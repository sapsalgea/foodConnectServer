...
$ArrayListSize = $_POST['ArrayListSize'];
...
$total_user_count_sql ="SELECT COUNT(*)FROM ranking_season_tb WHERE season_name = $now_season";
...
$sql = "SELECT *,(@rownum:=@rownum+1) AS number, (SELECT COUNT( * ) +1 FROM ranking_season_tb WHERE season_name = $now_season AND season_point > t.season_point) AS rank  FROM ranking_season_tb  AS t WHERE season_name = $now_season  ORDER BY season_point DESC limit $ArrayListSize,$limit_last_count";
        ...
        while($row = $result->fetch_assoc()){
          ...
          $top_tier_check_sql ="SELECT COUNT(*) FROM ranking_season_tb WHERE season_point>$rank_point AND season_name = $now_season";
