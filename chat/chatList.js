...
io.sockets.on('connection', (socket) => {
    ...
    socket.on('enter', (data) => {
        ...
        if (roomListUser.has(roomNumber)) {
            ...
            var userIndexSet = new Set()
            var userindex = socket.userIndex
            userIndexSet.add(userindex)
            roomListUser.set(roomNumber, userIndexSet)
    ...
    socket.on('listIn', (data) => {
        ...
        socket.userIndex = roomData.userIndex
        const roomNumber = roomData.roomId
        console.log(`9100 : ${socket.nickname}`)
        socket.join(`${roomNumber}`)
...
server.listen(9100, () => {
    console.log(`Server listening at http://localhost:9100`)
})